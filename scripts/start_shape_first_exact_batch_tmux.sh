...
SESSION="${SESSION:-cellular-shape-first}"
...
FORWARDED_ENV_KEYS=(
  ...
  SOLUTION_TICKS
  SOURCE_RATE
  MAX_SWAP_QUANTITY_PER_EDGE
  WIN_DURATION_TICKS
  REQUIRED_ALIVE_TICKS_AT_END
  SHAPE_FIRST_SUSTAINED_TICKS
  EVENT_CAPACITY
  CONFIGURATION
  HEARTBEAT_SECONDS
  PROGRESS_STRIDE
...
COMMAND="set -o pipefail; cd $quoted_pwd && env$ENV_PREFIX bash scripts/run_shape_first_exact_batch.sh $quoted_job 2>&1 | tee $quoted_log"
